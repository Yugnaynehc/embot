...
DEFAULT_HEAD = 'icons/chat.png'
HEAD1 = 'icons/head1.jpg'
...
class BubbleText(QLabel):
    ...
    def __init__(self, listItem, listView, text=DEFAULT_MSG, lr=True):
        ...
        if self.lr:
            ...
            self.setContentsMargins(self.trigon * sqrt(3) / 2 + 3,
                                    self.border + 3, self.border + 3, self.border + 3)
        else:
            self.setContentsMargins(self.border + 3, self.border + 3,
                                    self.trigon * sqrt(3) / 2 + 3, self.border + 3)
...
if __name__ == '__main__':
    ...
    ml = MsgList()
    ml.setMinimumSize(800, 600)
    ml.addTextMsg("Hello, 聊点啥？", False, HEAD1)
    ml.addTextMsg("World!", True, HEAD2)
    ml.show()
