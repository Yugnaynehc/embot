...
'''
from theano.gpuarray.basic_ops import ( #theano.gpuarray   theano.sandbox.cuda
                                           host_from_gpu,
...
def batchnorm(X, g=None, b=None, u=None, s=None, a=1., e=1e-8):
    ...
    if X.ndim == 4:
        ...
        if u is not None and s is not None:
            ...
            b_s = s.dimshuffle('x', 'x',0)
        else:
            b_u = T.mean(X, axis=[0, 1]).dimshuffle('x', 'x',0)
            b_s = T.mean(T.sqr(X - b_u), axis=[0, 1]).dimshuffle('x', 'x',0)
        if a != 1:
            b_u = (1. - a)*0. + a*b_u
            b_s = (1. - a)*1. + a*b_s
        X = (X - b_u) / T.sqrt(b_s + e)
        if g is not None and b is not None:
            X = X*g.dimshuffle('x', 'x',0)+ b.dimshuffle('x', 'x',0)
    elif X.ndim == 2:
